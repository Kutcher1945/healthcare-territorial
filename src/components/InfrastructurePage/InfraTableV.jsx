...
export default function InfraTable({ selectedDistrict, selectedDecade }) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Mapping: id -> visit (clinic-visit-5month)
      const idToVisit = {};
      arr4.forEach((item) => {
      ...
      const mergedData = arr1.map((item) => {
        ...
        return {
          ...
          totalPopulation: item.total_population ?? "N/A", // ✅ added from org-capacity
          ...
          visit: idToVisit[item.id] ?? "N/A", // ✅ added from clinic-visit-5month
          ...
          <tbody>
            {merged.map((item, index) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-center text-slate-700 border-b border-slate-200">
                  <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-purple-800">
